...
public class GetAlbumDescription {
    ...
    @RequestMapping(value = "/GetAlbumDescription", method = {RequestMethod.GET, RequestMethod.POST})
    protected void execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (request.getRemoteAddr().equals(request.getLocalAddr())) {
            ...
            String albumID = jsonObject.get("id").toString();

            Document getAlbum = Jsoup.connect("http://music.163.com/api/album/" + albumID + "?ext=true&id=" + albumID + "&offset=0&total=true&limit=1")
                    .userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.51 Safari/537.36")
                    .header("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8")
                    .header("Accept-Encoding", "gzip, deflate")
                    .header("Connection", "keep-alive")
            ...
            String json = getAlbum.body().text();
            JSONObject object = JSON.parseObject(json);
            object = JSON.parseObject(object.get("album").toString());
