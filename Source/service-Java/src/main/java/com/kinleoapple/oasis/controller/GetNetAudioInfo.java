...
import com.kinleoapple.oasis.tools.ToolUtils;
...
@Controller
public class GetNetAudioInfo {

    @RequestMapping(value = "/GetNetAudioInfo", method = {RequestMethod.GET, RequestMethod.POST})
        ...
        if (request.getRemoteAddr().equals(request.getLocalAddr())) {
            ...
            if (!id.equals("-1")) {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    JSONObject o = JSON.parseObject(jsonArray.get(i).toString());
                    artist.append(o.get("name").toString());
                ...
                name = ToolUtils.formatStr(name);
                artist = new StringBuilder(ToolUtils.formatStr(artist.toString()));
                album = ToolUtils.formatStr(album);
                String object = "{\"name\":\"" + name + "\",\"artist\":\"" + artist + "\",\"album\":\"" + album + "\"}";
