...
@Controller
public class GetNetAudioPicture {

    @RequestMapping(value = "/GetNetAudioPicture", method = {RequestMethod.GET, RequestMethod.POST})
    protected void execute(HttpServletRequest request, HttpServletResponse response, @RequestBody GetNetAudioPictureParameter parameter) throws IOException {
        if (request.getRemoteAddr().equals(request.getLocalAddr())) {
            ...
            if (!id.equals("-1")) {
                try {
                    ...
                    String url = jsonObject.get("picUrl").toString();
...
class GetNetAudioPictureParameter {
