...
@Controller
public class CheckHasRight {

    @RequestMapping(value = "/CheckHasRight", method = {RequestMethod.GET, RequestMethod.POST})
    protected void execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (request.getRemoteAddr().equals(request.getLocalAddr())) {
            ...
            boolean isOK;
            try {
                Document check = Jsoup.connect("https://autumnfish.cn/check/music?id=" + id)
                        ...
                        .timeout(20 * 1000)
                        .get();
                String json = check.body().text();
                isOK = JSON.parseObject(json).getBoolean("success");
            } catch (IOException e) {
                isOK = false;
            }
