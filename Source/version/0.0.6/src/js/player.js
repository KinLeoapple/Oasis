...
$(document).ready(function () {
    ...
    $('#delete_icon').html(SvgImage.deleteSvg());
    $('#download').hide();
...
function setLanguage() {
    ...
    $('#delete_text').html(languageList[10]);
        ...
        complete: function (res) {
            ...
            axios({
                ...
                Promise.resolve(respond.data).then(res => {
                    ...
                    audio[0].currentTime = json.process;
                    setCurrent(audio[0]);
                });
            });
            isConfig = true;
...
function setAudio(number, isLocal) {
    ...
    function afterComplete() {
        audio[0].oncanplay = function () {
            ...
            axios.post('http://localhost:23456/SetConfig?vol=' + volumeNumber * 100 + '&process=' + audio[0].currentTime + '&currentID=' + currentID + '&loopMode=' + loopMode + '&language=' + language + '&theme=' + theme);
        }
    }
...
// 创建频谱
function createSpectrum(audio) {
    try {
        let wrap = document.getElementById("spectrum");
...
function volumeBtn(audio) {
    ...
    $(document).on('mouseup', function () {
        if (isVolumeClick) {
            ...
            isVolumeClick = false;
            $('#volume_show').fadeOut(100);
            axios.post('http://localhost:23456/SetConfig?vol=' + volumeNumber * 100 + '&process=' + audio.currentTime + '&currentID=' + currentID + '&loopMode=' + loopMode + '&language=' + language + '&theme=' + theme);
        }
    });
...
function processBtn(audio) {
    ...
    $(document).on('mouseup', function () {
        if (isProcessClick) {
            ...
            audio.removeEventListener('timeupdate', scroll);
            ...
            if (currentLyricsLine < 4) {
                $('#inner_lyrics').css('margin-top', 0);
            } else {
                let total = 0;
                for (let i = 0; i <= currentLyricsLine - 4; i++) {
                    total += lineHeight[i] + 5;
                }
                $('#inner_lyrics').css('margin-top', -total + 'px');
            }
...
function setPlayList() {
    if (!isEmpty) {
        ...
        $('#list_content').html('<div class="empty_box" id="empty_box"></div>');
        $('#empty_box').html(SvgImage.emptyListSvg());
    }
...
function setLyricsScroll(audio) {
    audio.addEventListener('timeupdate', scroll);
}
...
function themeOptionBtn() {
    $('#skin').on('click', function () {
        optionCommon('skin');
        $('#setting_panel').html('<div id="theme_block">\n' +
            '<div class="setting_panel_title" id="switch_theme"></div>\n' +
            '<div class="setting_block">\n' +
            '<div id="default">' + languageList[26] + '</div>\n' +
            '<div id="pink">' + languageList[27] + '</div>\n' +
            '</div>\n' +
            '</div>');
        $('#switch_theme').html(languageList[22]);
        if (settingpanel_scrollbar) settingpanel_scrollbar.destroy();
        settingpanel_scrollbar = new PerfectScrollbar('#setting_panel');
...
function aboutOptionBtn() {
    $('#about').on('click', function () {
        ...
        $('#setting_panel').html('<div id="about_block">\n' +
            ...
            marked.parse('<h1> Oasis @0.0.6</h1><br>\n' +
                '<h3> ✨ Features ✨ </h3><br>\n' +
                '<ol>\n' +
